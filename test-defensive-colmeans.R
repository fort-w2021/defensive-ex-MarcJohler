...
test_that("deals with missings", {
  ...
  expect_equal(
    col_means(mtcars_NA)[, "mpg"],
    NA_real_
  )
  expect_equal(
    col_means(mtcars_NA)[, -1],
    col_means(mtcars)[, -1]
  )
  expect_equal(
    col_means(mtcars_NA, na.rm = TRUE)[, "mpg"],
    col_means(mtcars[-(1:10), ])[, "mpg"]
  )
  expect_equal(
    col_means(mtcars_NA, na.rm = TRUE)[, -1],
    col_means(mtcars)[, -1]
  )
})
...
if (TRUE) {
  ...
  test_that(
    "deals with date variables as well",
    expect_equal(
      col_means(mtcars_date),
      cbind(col_means(mtcars),
        date = mean(mtcars_date$date)
      )
    )
  )
